...
<div id="usual1" class="usual"> 
    <div class="itab">
  	<ul> 
    <li><a href="#tab1" class="selected">滚动图编辑</a></li> 
  	</ul>
</div> 
<div id="tab1" class="tabson">
  <ul class="forminfo">
					<form action="" method="post" id="systemAdvForm">
						...
						<input type="hidden" class="dfinput" id="id" name="systemAdv.id" placeholder="请输入id" value="${ systemAdv.id}" />
						<li><label>标题<b></b></label> <input type="text"
							class="dfinput" id="title" name="systemAdv.title"
							placeholder="请输入标题" value="${ systemAdv.title}" /></li>
						<li><label>简介<b></b></label> 
							...
							<input type="checkbox" name="showFlag" <c:if test="${systemAdv.showFlag==1 }">checked</c:if>/>标题与简介是否显示 
							</li>
						...
						<li><label>wap跳转地址<b></b></label> <input type="text"
						 ...
						 <li><label>首页图<b></b></label> <input type="text"
							...
							<img id="indeximgShow" height="30" width="30" style="vertical-align:middle;"    src="${systemAdv.imgUrl }"/><i>建议尺寸：1920*600</i> 
						  </li> 
						<li><label>排序号<b></b></label> <input type="text"
							class="dfinput" id="orderlist" name="systemAdv.orderlist"
							placeholder="请输入排序号" value="${ systemAdv.orderlist}" /></li>
						<li><label>是否启用<b></b></label>
						<div class="vocation">
								<select name="systemAdv.status" class="select1">
									<option value="1" <c:if test="${systemAdv.status==1 }">selected</c:if>>启用</option>
									<option value="0" <c:if test="${systemAdv.status==0 }">selected</c:if>>停用</option>
								</select>
							</div>
						</li>
  ...
  <script type="text/javascript">
  	$("#abstractContent").html("${systemAdv.abstractContent}");
 	...
 	$(document).ready(
 		function(){
 		  $(".select1").uedSelect({
 			...
 			$('#systemAdvForm').on("click", ".doSubmit", function(e){
		...
		$.ajax({
				url : "${ contextpath}/systemadv/modifyaction",
				data : $(form)
						.serialize(),
				type : "POST",
				success : function(
					...
					if (data.status == 'success') {
						parent.layer
								.alert(
										...
										function(
												index) {
											window.location.href = '${ contextpath}/toIndexSetting.htm';
											parent.layer
													.close(index);
										});
					} else {
