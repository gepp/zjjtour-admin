						...
						<li><label>是否启用<b></b></label>
							<div class="vocation">
								<select name="member.status" class="select1">
									<option value="0"
										<c:if test="${member.status==0 }">selected</c:if>>停用</option>
									<option value="1"
										<c:if test="${member.status==1 }">selected</c:if>>启用</option>
								</select>
							</div></li>
