...
<body>
	...
	<div class="formbody">
		<div id="usual1" class="usual">
			<div class="itab">
				<ul>
					<li><a href="#tab1" class="selected">用户编辑</a></li>
				</ul>
			</div>
			<div id="tab1" class="tabson">
				<ul class="forminfo">
					<form action="" method="POST" id="securityUserForm">
						...
						<li><label>拥有角色<b></b></label> <input type="text" class="dfinput" id="roleNames" name="roleNames" placeholder="" readonly value="${roleNames }" /> <input type="button" value="选择角色" onclick="selectRole();" class="btn" /></li>
						<li><label>状态<b></b></label>
							<div class="vocation">
								<select name="securityRole.status" class="select1">
									<option value="0" <c:if test="${ securityUser.status==0}">selected</c:if>>停用</option>
									<option value="1" <c:if test="${ securityUser.status==1}">selected</c:if>>启用</option>
								</select>
							</div></li>
								...
								.validator(
										{
											...
											valid : function(form) {
												...
												me.holdSubmit();
												$.ajax({
															url : "${ contextpath}/securityuser/modifyaction",
															data : $(form)
																	.serialize(),
															type : "POST",
															success : function(
